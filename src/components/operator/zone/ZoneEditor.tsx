...
export function ZoneEditor({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center" style={{ background: "rgba(0,0,0,0.6)" }} onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="w-[min(92vw,980px)] max-h-[92vh] overflow-auto rounded-2xl flex flex-col" style={{ background: PANEL, border: `1px solid ${AMBER}33`, boxShadow: "0 24px 80px rgba(0,0,0,0.6)" }}>
        {/* Header + profile bar */}
        <div className="flex items-center justify-between gap-3 px-5 py-3 border-b" style={{ borderColor: "#ffffff14" }}>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button className="text-[12px] px-2 py-1.5 rounded-md" style={{ color: AMBER, border: `1px solid ${AMBER}55` }}
              onClick={() => { const n = window.prompt("New profile name", "New profile"); if (n) store.create(n, true); }}>+ New</button>
            <button className="text-[12px] px-2 py-1.5 rounded-md text-white/70 hover:text-white" style={{ border: "1px solid #ffffff1a" }}
              onClick={() => { const n = window.prompt("Rename profile", active.name); if (n) store.rename(active.id, n); }}>Rename</button>
            <button className="text-[12px] px-2 py-1.5 rounded-md text-white/70 hover:text-white" style={{ border: "1px solid #ffffff1a" }}
            ...
            <button className="text-[12px] px-2 py-1.5 rounded-md text-red-300/80 hover:text-red-300" style={{ border: "1px solid #ffffff1a" }}
              ...
              onClick={() => { if (window.confirm(`Delete "${active.name}"?`)) store.remove(active.id); }}>Delete</button>
          </div>
        ...
        <div className="p-5">
          ...
          <div className="mt-2 text-[11px] text-white/40">Drag to move · drag a handle to resize · changes are live on your projector. Amber dashes = 5% overscan safe area.</div>
        </div>
