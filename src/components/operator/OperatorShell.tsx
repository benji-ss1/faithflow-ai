        ...
        <div className="flex-1 min-h-0 flex">
          ...
          <div className="flex flex-col min-h-0">
            <div className="shrink-0 p-2 border-b border-l" style={{ borderColor: "#2a3232", background: "#1a2020" }}>
              <LiveOutputThumb liveSlide={ctx.liveSlide} appearance={ctx.liveAppearance ?? ctx.appearance} fontScale={ctx.fontScale} background={ctx.background} videoInput={ctx.videoInput} {...(ctx.layerOrderV3 ? { layerOrderV3: true, ...(ctx.themeLayerHidden ? { themeLayerHidden: true } : {}), announcement: ctx.announcement, zone: ctx.zone ?? null, aspectRatio: ctx.aspectRatio, ...(ctx.layersEngineOn ? { layerOverrides: ctx.liveLayers.overrides } : {}) } : {})} />
            </div>
            <div className="flex-1 min-h-0 flex">
