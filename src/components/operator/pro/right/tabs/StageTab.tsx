...
const RESOLUTIONS = [
  ...
  "4Kp25", "4Kp30", "4Kp50", "4Kp60",
];
const RESOLUTION_KEY = "presentflow.stage.resolution";

export function StageTab({ ctx: _ctx }: { ctx: OperatorShellCtx }) {
  const [displays, setDisplays] = useState<DisplayInfo[] | null>(null);
  const [resolution, setResolution] = useState<string>(RESOLUTIONS[5]);

  useEffect(() => {
    try {
      const raw = window.localStorage.getItem(RESOLUTION_KEY);
      if (raw && RESOLUTIONS.includes(raw)) setResolution(raw);
    } catch { /* noop */ }
    // Fallback: if there's no Electron bridge, mark displays as "unavailable"
    // after 500ms instead of showing "Detecting displays…" forever.
    if (typeof window === "undefined" || !window.electronAPI) {
      const t = setTimeout(() => setDisplays([]), 500);
      return () => clearTimeout(t);
    }
    void window.electronAPI.screens.list().then(setDisplays);
  }, []);

  const onResolutionChange = (v: string) => {
    setResolution(v);
    try { window.localStorage.setItem(RESOLUTION_KEY, v); } catch { /* noop */ }
  };
    ...
    <div className="flex flex-col gap-3">
      ...
      <div>
        ...
        {displays === null ? (
          ...
          <div className="text-[var(--color-muted-foreground)] py-2 text-[11px]">
            {typeof window !== "undefined" && window.electronAPI
              ? "No displays detected."
              : "Display list is only available in the Present Flow desktop app."}
          </div>
        ) : (
