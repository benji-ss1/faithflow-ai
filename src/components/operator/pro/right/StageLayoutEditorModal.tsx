 ...
 *
 * Three columns at >=1100px (a 1366x768 church laptop clears it); one column
 * below that, because three columns in a small window is worse than none.
 */
...
function rowLabel(w: StageWidget, timers: TimersApi): { text: string; muted?: string } {
  if (w.kind === "timer") {
    const name = w.timerId ? timers.slots.find((s) => s.def.id === w.timerId)?.def.name : null;
    return { text: "Timer", muted: name ?? "not chosen" };
  }
...
export function StageLayoutEditorModal({
  open, layout, timers, screenName, isCopyOfBuiltIn, onClose, onSave,
}: {
  open: boolean;
  layout: StageLayout;
  timers: TimersApi;
  ...
  const duplicate = (id: string) => {
    ...
    if (!src || draft.widgets.length >= STAGE_MAX_WIDGETS) return;
    const copy: StageWidget = {
      ...src,
      id: `w${Date.now().toString(36)}`,
      rect: clampRect({ ...src.rect, x: src.rect.x + 0.03, y: src.rect.y + 0.03 }),
      zIndex: draft.widgets.length,
    };
    edit({ ...draft, widgets: [...draft.widgets, copy] });
    setSel(copy.id);
  };
  ...
  const order = (id: string, dir: 1 | -1) => {
    const sorted = [...draft.widgets].sort((a, b) => a.zIndex - b.zIndex);
    const i = sorted.findIndex((w) => w.id === id);
    const j = i + dir;
    if (i < 0 || j < 0 || j >= sorted.length) return;
    [sorted[i], sorted[j]] = [sorted[j], sorted[i]];
    edit({ ...draft, widgets: sorted.map((w, k) => ({ ...w, zIndex: k })) });
  };
  ...
  const remove = async (id: string) => {
    ...
    edit({ ...draft, widgets: draft.widgets.filter((x) => x.id !== id) });
              ...
              {[...draft.widgets].sort((a, b) => b.zIndex - a.zIndex).map((w) => {
                ...
                return (
                  <div key={w.id}
                    ...
                    <button onClick={() => order(w.id, 1)} aria-label="Bring forward" className="w-5 h-5 flex items-center justify-center opacity-60 hover:opacity-100">
                      <ChevronUp className="w-3 h-3" />
                    </button>
                    <button onClick={() => order(w.id, -1)} aria-label="Send back" className="w-5 h-5 flex items-center justify-center opacity-60 hover:opacity-100">
                      <ChevronDown className="w-3 h-3" />
                    </button>
                    <button onClick={() => duplicate(w.id)} aria-label="Duplicate" className="w-5 h-5 flex items-center justify-center opacity-60 hover:opacity-100">
                      <Copy className="w-3 h-3" />
                    </button>
                    <button onClick={() => { void remove(w.id); }} aria-label="Remove"
                      className="w-5 h-5 flex items-center justify-center text-[var(--color-destructive)]">
              ...
              {draft.widgets.some((w) => w.kind === "timer" && !w.timerId) && (
                <div className="text-[11px] text-[var(--color-destructive)] max-w-[600px] text-center">
                  {timers.slots.length === 0
                    ...
                    : "A timer box has no timer chosen, so the stage screen will show a dash. Pick one on the right."}
                </div>
              )}
            </div>
