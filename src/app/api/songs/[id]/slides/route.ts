...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // `themed`: the song carries a baked theme (settings.appliedThemeId) — the
  // Songs browser then previews/projects the slide's stored background.
  const themed = typeof ((song.settings as Record<string, unknown> | null) ?? {}).appliedThemeId === "string";
  return NextResponse.json({ title: song.title, themed, slides: slides.map((s) => ({ id: s.id, order: s.order, lyrics: sanitizeLyrics(s.lyrics), objectsJson: s.objectsJson })) });
}
