/**
 * Public-domain hymn library — shared source of truth for both the CLI
 * seed script (scripts/seed-hymns.ts) and the in-app "Add hymn library"
 * onboarding step (server action `addBuiltInHymnsToMyChurch` in actions.ts).
 *
 ...
 * Sources for lyrics: Cyber Hymnal (hymnary.org) and Wikisource — both host
 * lyrics only after clearing PD status themselves.
 *
 ...
 *
 * When a hymn's `slides` array is empty, the entry is a deliberate skip —
 * documented in `verified` — and consumers must NOT insert it.
 */
