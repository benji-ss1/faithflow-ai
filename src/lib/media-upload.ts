 ...
 * back a durable URL. Reused by the Layers logo picker (wave 6F rec5) AND the
 * Themes BgAssetPicker (wave-6 fix pass) so they share ONE upload path rather
 * than duplicating it.
 *
 * NOTE: named `uploadFileToMediaStorage` (not `uploadMediaFile`) to avoid a name
 * collision with MediaImportWizard's local `uploadMediaFile`, which is a
 * different flow (library import: presign → PUT → server-side registerMediaAsset,
 * no signed-URL return). The other legacy per-surface presign flows are tracked
 * for consolidation in docs/DECOUPLING_PLAN.md (Media upload consolidation note).
 *
...
export async function uploadFileToMediaStorage(file: File, purpose: UploadPurpose = "media"): Promise<string> {
  ...
  const presign = await presignRes.json() as { url?: string; key?: string; error?: string };
  ...
  if (!urlRes.ok) throw new Error(`Could not get download URL (${urlRes.status})`);
  const got = await urlRes.json() as { url?: string; error?: string };
...
export function uploadImageFile(file: File, purpose: UploadPurpose = "logo"): Promise<string> {
  return uploadFileToMediaStorage(file, purpose);
}
