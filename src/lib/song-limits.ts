 ...
 *  Free/gated tier stays at 50 — real accounts must pay (or buy bundles) to go
 *  beyond. Max/pilot subscribers and DEMO churches are lifted to effectively
 *  unlimited in getEffectiveSongLimit() (server-only), so the freemium gate is
 *  preserved for real free accounts while the demo has everything. */
