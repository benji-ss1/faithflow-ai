...
export function skippedSummary(skipped: Array<{ name: string; route: DroppedFileRoute["route"] }>): string | null {
  ...
  if (by("heic").length) parts.push(`${list(by("heic"))}: iPhone HEIC photos aren't supported yet — export as JPG`);
 ...
 */
export async function collectDroppedFiles(dt: DataTransfer, max = MEDIA_BIN_MAX_FILES): Promise<File[]> {
  ...
  if (entries.length === 0) return Array.from(dt.files ?? []).slice(0, max);
  const plain = Array.from(dt.files ?? []);

  const out: File[] = [];
  const walk = async (e: FsEntry, depth: number): Promise<void> => {
    if (out.length >= max || e.name.startsWith(".")) return;
    if (e.isFile && e.file) {
  ...
  return out.length > 0 ? out : plain.slice(0, max);
}
